...
void loop()
{
  ...
  while(waitingInput and Serial.available() > 0)
  {
    ...
    int distsub = data.substring(1).toInt();

    //Serial.println(distsub);
    
    if(readChar == 'F')
    {
      if(distsub > 0)
      {
        Front_Back_ticks = distsub / 10 * Fixed_Front_Back + Fixed_Front_Back;
      }
      else
      {
        Front_Back_ticks = Fixed_Front_Back;
      }
    }
    ...
    if(readChar == 'L')
    {
      if(distsub > 0)
      {
        //Left_ticks = distsub / 90 * Fixed_Left;
        //Left_ticks = 4.5553 * distsub -16.00;
        Left_ticks = 0.00034 * distsub + 4.15206 * distsub + 58.5302;
      }
      else
      {
        //Left_ticks = Fixed_Left;
        Left_ticks = 0.00034 * 90 + 4.15206 * 90 + 58.5302;
      }
    }
    ...
    if(readChar == 'R')
    {
      if(distsub > 0)
      {
        Right_ticks = 0.00034 * distsub + 4.15206 * distsub + 58.5302;
      }
      else
      {
        //Right_ticks = Fixed_Right;
        Right_ticks = 0.00034 * 90 + 4.15206 * 90 + 58.5302;
      }
    }

    Serial.print("Set distance") ; Serial.println(Left_ticks);
