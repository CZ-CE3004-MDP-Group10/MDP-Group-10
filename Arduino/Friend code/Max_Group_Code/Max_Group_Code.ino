...
/* Variables for PID */
const double kP_1 = 1, kI_1 = 0, kD_1 = 0; // Spd 300 | P=13, I=0.85, D=0;
const double kP_2 = 0, kI_2 = 0, kD_2 = 0;    // to find out

/* PID FORMAT: input | output | setpoint | PID params ===> PID COMPUTES output to be added to input to equalize with setpoint */ 
...
int RightTurn_90_Ticks = 381;       // to find out PREV 383 385 , Spd 350 381
...
void Forward(int cm, int iAmSpeed){
  ...
  while( M1_Ticks <= dist || M2_Ticks <= dist){
    if(dist - M2_Ticks < 100) // INITIAL 150
      currentSpeed = 200; // start slower initially before picking up speed 
    if((M1_Ticks - last_tick_R) >= 10 || M1_Ticks == 0 || M1_Ticks == last_tick_R){  //m2 || m2 || m2
      last_tick_R = M1_Ticks;
      offset += 0.1;
    }
    if(myPID.Compute()||M1_Ticks == last_tick_R){ 
      if(offset >= 1){
